...
export class Player {
  ...
  public registerHTMLListeners(): void {
    ...
    for (let i = 0; i < deckOptionRadios.length; i++) {
      ...
      deckOptionRadio.onchange = () => {
        ...
        const deckElement = <HTMLInputElement> document.querySelector(deckQuery);
    ...
    const timerElement = <HTMLInputElement> document.getElementById(timerQuery);
  ...
  private setDragHandlers(target): void {
    ...
    const hintDragDrop = (evt) => {
      ...
      const other = <HTMLImageElement> document.getElementsByName('dragging').item(0);
      other.name = null;
  ...
  public validateHints(): void {
    ...
    if (handCards < 1) {
      ...
      error.innerText = 'You can only play one card from your hand each round';
    } else {
      error.innerText = '';
      disable = false;
    }

    const button = <HTMLInputElement> document.getElementById('btnCommitHints');
  ...
  public tick(): void {
    this.timerCount--;
    document.getElementById('timerDisplay').innerText = Number(this.timerCount).toString();
  ...
  setUserHandlers(): void {
    ...
    this.client.on('lateArrival', (state) => { 
      ...
      if (state.mode == 'coop') {
        ...
        const teamOneBtn = <HTMLInputElement> document.getElementById('btnJoinTeamOneLate');
        teamOneBtn.value = teamOneLabel;

        const teamTwoLabel = state.teams.names[1];
        const teamTwoBtn = <HTMLInputElement> document.getElementById('btnJoinTeamTwoLate');
  ...
  public assignToastmaster(amNowToastmaster): void {
    ...
    if (this.isToastmaster) {
      ...
      const hostElements = document.getElementsByClassName('clientVisible');
  ...
  public setWait(game: GameState): void {
    ...
    if (this.waiting) {
      const elementsToHide = ['hintsStage', 'discussStage', 'btnEndGuessing',
        'handContainer'];
  ...
  public renderLobby(game: GameState): void {
    ...
    if (game.mode == 'coop') {
      document.getElementById('teamOneListContainer').classList.add('hidden');
      document.getElementById('teamTwoListContainer').classList.add('hidden');

      modeDisplayText = 'Cooperative';
      const userListContainer = document.getElementById('userListContainer');
      userListContainer.classList.remove('userListFloat');
      this.listPlayers(game.players, 'userList', 'players', game.toastmaster);

      document.getElementById('btnChangeToCoop').classList.add('hidden');
      document.getElementById('btnChangeToTeams').classList.remove('hidden');


      this.team = -1;
    } else {
      modeDisplayText = 'Teams';
      document.getElementById('teamOneListContainer').classList.remove('hidden');
      document.getElementById('teamTwoListContainer').classList.remove('hidden');
      this.listPlayers(game.teams.unassigned, 'userList', 'unassigned', null);

      this.listPlayers(game.teams.team[0], 'teamOneList', 
                          'teamOne', game.teams.toastmasters[0]);
      this.listPlayers(game.teams.team[1], 'teamTwoList',
                          'teamTwo', game.teams.toastmasters[1]);

      document.getElementById('btnChangeToTeams').classList.add('hidden');
      document.getElementById('btnChangeToCoop').classList.remove('hidden');
      
      const joinTeamButtons = document.getElementsByClassName('btnJoinTeam');

      joinTeamButtons.item(0).classList.remove('hidden');
      joinTeamButtons.item(1).classList.remove('hidden');

      if (this.team == 0) {
        const joinTeamButton = document.getElementById('btnJoinTeamOne');
        joinTeamButton.classList.add('hidden');
      } else if (this.team == 1) {
        const joinTeamButton = document.getElementById('btnJoinTeamTwo');
        joinTeamButton.classList.add('hidden');
      }
    }
    ...
    if (game.deck == 'basic') {
      ...
      if (this.isToastmaster) {
        ...
        let deckElement = <HTMLInputElement> document.getElementById(deckQuery);
        deckElement.checked = true;
        deckQuery = 'rdDeckAdvanced';
        deckElement = <HTMLInputElement> document.getElementById(deckQuery);
  ...
  public renderDiscussStage(game: GameState): void {
    ...
    if (game.timer) {
      ...
      if (this.isToastmaster) {
        document.getElementById('btnEndGuessing').classList.add('hidden');
      }
    } else {
      if (this.isToastmaster) {
        document.getElementById('btnEndGuessing').classList.remove('hidden');
      }
    }
  ...
  public point(name): void {
    ...
    const btnUnpoint = container.getElementsByClassName('btnUnpoint').item(0)
  ...
  public unpoint(): void {
    const target = <HTMLImageElement> document.getElementsByName('pointed').item(0);
    if (target == null) {
      return;
    }
    ...
    const unpointButtonList = container.getElementsByClassName('btnUnpoint');
